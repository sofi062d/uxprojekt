...
$(document).ready(function () {
    ...
    $("#markedf").click(function () {
        $("#myTextBox").val("Market Economy").change();
        $("#submit").trigger("click");
        $("#accordion_section").show();
    });

    $("#webdesign").click(function () {
        $("#myTextBox").val("Web Design").change();
        $("#submit").trigger("click");
        $("#accordion_section").show();
    });

    $("#grafisk").click(function () {
        $("#myTextBox").val("Graphic design").change();
        $("#submit").trigger("click");
        $("#accordion_section").show();
    });

    $("#multi").click(function () {
        $("#myTextBox").val("Multimedia design").change();
        $("#submit").trigger("click");
        $("#accordion_section").show();
    });
    ...
    $('#submit').click(function () {
        ...
        $.ajax({
            ...
            success: function (xml) {
                ...
                let i = 0;
                $("#overskrift").html("Books about " + $("#myTextBox").val());
                ...
                $(xml).find('item').each(function () { // ved hver <item> gøres følgende ...
                    ...
                    $(paneltekst[i]).html('<div class="bog">' + // .bog en bogkasse begynder
                        ...
                        '<h3>' + titel + '</h3>' + // titel
                        '<h4> Author: ' + forfatter + '</h4>' + // forfatter
                        // visBillede( billede ) + // skriver kun billedtag, hvis der er et billede
                        '<p>Description: ' + beskrivelse + '</p>' + // beskrivelse
                        '<p>Release: ' + pubdate.substring(0, 16) + '</p>' + // tryk: dato
